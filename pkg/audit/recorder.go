...
func (r *Recorder) Write(ctx context.Context, actor string, old, new *registry.FieldMeta) error {
	...
	unified, addCnt, delCnt := UnifiedDiff(before, after)
	summary := fmt.Sprintf("+%d -%d", addCnt, delCnt)
	beforeNorm := NormalizeJSON(before)
	afterNorm := NormalizeJSON(after)
	lines := strings.Split(unified, "\n")
	if len(lines) > 20 {
		lines = lines[:20]
	}
	if enableVerboseAuditLogs {
		log.Printf("audit diff summary=%s before=%s after=%s added=%d removed=%d diff=%s",
			summary, beforeNorm, afterNorm, addCnt, delCnt, strings.Join(lines, "\n"))
	}
