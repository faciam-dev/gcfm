...
func TestExportLocal(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"table_name", "column_name", "data_type"}).AddRow("posts", "title", "text")
	mock.ExpectQuery("^SELECT table_name, column_name, data_type FROM custom_fields ORDER BY table_name, column_name$").WillReturnRows(rows)
	dir := t.TempDir()
	if err := snapshot.Export(context.Background(), db, "", snapshot.LocalDir{Path: dir}); err != nil {
