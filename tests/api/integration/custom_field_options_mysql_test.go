...
func TestAPI_Create_CF_Options_MySQL(t *testing.T) {
	...
	row := db.QueryRowContext(ctx, `SELECT IS_NULLABLE, COLUMN_DEFAULT FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='testdb' AND TABLE_NAME='posts' AND COLUMN_NAME='email'`)
	var isNullable, defVal string
	if err := row.Scan(&isNullable, &defVal); err != nil {
	...
	if defVal != "abc" {
		t.Fatalf("default mismatch: %s", defVal)
	}

	row = db.QueryRowContext(ctx, `SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA='testdb' AND TABLE_NAME='posts' AND COLUMN_NAME='email' AND NON_UNIQUE=0`)
	var cnt int
	if err := row.Scan(&cnt); err != nil {
		t.Fatalf("unique check: %v", err)
	}
	if cnt == 0 {
		t.Fatalf("unique constraint missing")
	}
}
