...
func TestAddColumnSQL_MySQL_TimeExpression(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("sqlmock: %v", err)
	}
	add := "ALTER TABLE `posts` ADD COLUMN `opened` TIME DEFAULT (CURRENT_TIME)"
	mock.ExpectExec(regexp.QuoteMeta(add)).WillReturnResult(sqlmock.NewResult(0, 1))
	d := registry.UnifiedDefault{Mode: "expression", Raw: "curtime()", OnUpdate: true}
	if err := registry.AddColumnSQL(context.Background(), db, "mysql", "posts", "opened", "TIME", nil, nil, d); err != nil {
		t.Fatalf("add: %v", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatalf("unmet: %v", err)
